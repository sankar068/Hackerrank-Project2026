...
function UsersPage() {
  ...
  const users: UserAgg[] = useMemo(() => {
    ...
    return Array.from(map.values()).map((u) => ({
      ...u,
      riskLevel:
        u.rejected >= 2 || u.recent >= 3
          ? "High"
          : u.rejected >= 1 || u.manual >= 1
            ? "Medium"
            : "Low",
    }));
  }, [claims]);
  ...
  return (
    <div className="space-y-5">
      <p
        className="rounded-md border border-ai/30 bg-ai/5 p-3 text-sm"
        style={{ color: "var(--ai)" }}
      >
        User history is contextual risk information. It must not override clear visual evidence by
        itself.
      </p>
      <Card>
        <CardHeader>
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/40 text-xs uppercase tracking-wide text-muted-foreground">
              <tr>
                ...
                <th className="px-3 py-2.5 text-center">Manual</th>
                <th className="px-3 py-2.5 text-center">Recent</th>
                <th className="px-3 py-2.5 text-left">Risk</th>
                <th className="px-3 py-2.5 text-left">Last claim</th>
                <th></th>
              </tr>
            </thead>
            <tbody>
              ...
              {users.map((u) => (
                <tr key={u.userId} className="border-t border-border hover:bg-muted/30">
                  ...
                  <td className="px-3 py-2.5 text-center" style={{ color: "var(--destructive)" }}>
                    {u.rejected}
                  </td>
                        ...
                        color:
                          u.riskLevel === "High"
                            ...
                            : u.riskLevel === "Medium"
                              ? "oklch(0.45 0.13 75)"
                              : "var(--success)",
                        backgroundColor: `color-mix(in oklab, ${u.riskLevel === "High" ? "var(--destructive)" : u.riskLevel === "Medium" ? "var(--warning)" : "var(--success)"} 12%, transparent)`,
                      }}
                    >
                      {u.riskLevel}
                    </span>
                  ...
                  <td className="px-3 py-2.5 text-xs text-muted-foreground">
                    {format(new Date(u.lastClaim), "PP")}
                  </td>
    ...
    <Dialog>
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>
            {user.userName}{" "}
            <span className="ml-2 font-mono text-xs text-muted-foreground">{user.userId}</span>
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
          ...
          {user.claims.map((c) => (
            ...
            >
              <div>
                <div className="font-mono text-xs text-muted-foreground">{c.claimId}</div>
                <div className="font-medium">{c.claimTitle}</div>
