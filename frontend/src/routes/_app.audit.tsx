...
function AuditPage() {
  ...
  const filtered = items.filter((i) => {
    ...
    if (q && !`${i.claimId} ${i.reason} ${i.newValue}`.toLowerCase().includes(q.toLowerCase()))
      return false;
    return true;
  });
  ...
  return (
    <div className="space-y-5">
      <Card>
        <CardContent className="grid gap-3 p-4 md:grid-cols-4">
          <Input
            ...
            placeholder="Search by claim, reason, value"
          />
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Append-only audit log ({filtered.length})</CardTitle>
        </CardHeader>
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/40 text-xs uppercase tracking-wide text-muted-foreground">
              <tr>
                ...
                <th className="px-3 py-2.5 text-left">Previous</th>
                <th className="px-3 py-2.5 text-left">New</th>
                        ...
                        color:
                          e.status === "ok"
                            ...
                            : e.status === "warning"
                              ? "oklch(0.45 0.13 75)"
                              : "var(--destructive)",
                        backgroundColor: `color-mix(in oklab, ${e.status === "ok" ? "var(--success)" : e.status === "warning" ? "var(--warning)" : "var(--destructive)"} 12%, transparent)`,
                      }}
                    >
                      {e.status}
                    </span>
