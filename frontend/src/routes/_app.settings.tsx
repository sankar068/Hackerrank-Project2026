...
function SettingsPage() {
  const [policy, setPolicy] = useState({
    autoManualHighRisk: true,
    requireApprovalContradicted: true,
    requireApprovalManipulation: true,
    requireMoreEvidenceMissing: true,
  });
  return (
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-2">
          <div>
            ...
            <Input defaultValue="Northwind Claims Co." />
          </div>
          <div>
            <Label>Default reviewer</Label>
            <Input defaultValue="Review Administrator" />
          </div>
          <div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {[
            ...
            [
              "requireMoreEvidenceMissing",
              "Require additional evidence for missing-content claims",
            ],
          ].map(([k, l]) => (
            <div
              key={k}
            ...
            >
              ...
              <Switch
                checked={(policy as never)[k]}
                onCheckedChange={(v) => setPolicy({ ...policy, [k]: v })}
              />
            </div>
          ))}
        </CardContent>
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-2 opacity-70">
          ...
          <div>
            ...
            <Input disabled defaultValue="30" />
          </div>
        </CardContent>
        <div className="px-6 pb-6 text-xs text-muted-foreground">
          Model integration will be connected to the production backend later.
        </div>
      </Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {CSV_COLUMNS.map((c) => (
              <code
                key={c}
                className="rounded-md border border-border bg-muted px-2 py-1 text-[11px]"
              >
                {c}
              </code>
            ))}
