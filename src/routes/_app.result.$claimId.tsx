...
function ResultPage() {
  ...
  const bannerCfg =
    result.claimStatus === "supported"
      ? {
          Icon: CheckCircle2,
          text: "The submitted images support the user's claim.",
          bg: "var(--success)",
        }
      : result.claimStatus === "contradicted"
        ? {
            Icon: XCircle,
            text: "The visible image evidence contradicts the user's claim.",
            bg: "var(--destructive)",
          }
        : {
            Icon: AlertTriangle,
            text: "The submitted images do not provide enough information to verify the claim.",
            bg: "var(--warning)",
          };
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-5">
      ...
      {/* Banner */}
      <div
        className="rounded-2xl p-5 text-white shadow-elevated"
        style={{
          background: `linear-gradient(135deg, ${bannerCfg.bg}, color-mix(in oklab, ${bannerCfg.bg} 70%, black))`,
        }}
      >
        <div className="flex items-start gap-4">
          ...
          <span className="rounded-full border border-white/30 bg-white/10 px-2.5 py-1 text-[11px] font-medium">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <FieldCard
          label="Evidence standard met"
          value={result.evidenceStandardMet ? "True" : "False"}
          valueColor={result.evidenceStandardMet ? "var(--success)" : "var(--destructive)"}
        />
        <FieldCard
          label="Valid image"
          value={result.validImage ? "True" : "False"}
          valueColor={result.validImage ? "var(--success)" : "var(--destructive)"}
        />
        ...
        <FieldCard
          label="Supporting image IDs"
          value={result.supportingImageIds.join(", ") || "none"}
          mono
        />
        <FieldCard label="Reviewed" value={new Date(result.reviewedAt).toLocaleString()} />
        <FieldCard
          label="Evidence standard reason"
          value={result.evidenceStandardMetReason}
          fullSpan
        />
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Extracted Claim</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2 text-sm">
            ...
            {result.extractedClaim.locationQualifier && (
              <Row label="Location qualifier" value={result.extractedClaim.locationQualifier} />
            )}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Evidence Requirement Check</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2.5 text-sm">
            <div className="rounded-md bg-muted p-2.5 text-xs">
              <b>Expected:</b> {result.minimumEvidenceExpected}
            </div>
            {result.evidenceChecks.map((c) => (
              <div key={c.label} className="flex items-center justify-between">
                ...
                <span
                  ...
                  style={{ color: c.passed ? "var(--success)" : "var(--destructive)" }}
                >
                  {c.passed ? (
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            {result.riskFlags.includes("text_instruction_present") && (
              ...
              >
                Security alert: embedded instructions detected in the conversation. They were
                ignored by the review engine.
              </div>
            )}
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Image Evidence Review</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {claim.images.map((img) => {
                ...
                return (
                  <div
                    key={img.id}
                    className={`rounded-xl border bg-card p-3 ${supports ? "border-primary ring-1 ring-primary/20" : "border-border"}`}
                  >
                    {img.dataUrl && (
                    ...
                    {r && (
                      <div className="mt-2 space-y-1 text-xs text-muted-foreground">
                        <div>
                          <span className="text-foreground font-medium">Object:</span>{" "}
                          {r.detectedObject} ·{" "}
      ...
      <Collapsible className="rounded-xl border border-border bg-card">
        ...
        <CollapsibleContent className="border-t border-border px-4 py-4 text-sm space-y-3">
          <Concept
            t="Evidence Standard Met"
            d="Whether the images contain enough information to evaluate the claim."
          />
          <Concept
            t="Valid Image"
            d="Whether the submitted image is usable and trustworthy for automated evidence review."
          />
          <Concept
            t="Claim Status"
            d="The final decision about whether the images support, contradict or cannot verify the claim."
          />
          <p className="rounded-md bg-muted p-3 text-xs text-muted-foreground">
            Evidence sufficiency, image validity and claim truth are separate concepts.
          </p>
        </CollapsibleContent>
      </Collapsible>
      ...
      <Collapsible className="rounded-xl border border-border bg-card">
        <CollapsibleTrigger className="flex w-full items-center justify-between p-4 text-left">
          <span className="font-semibold flex items-center gap-2">
            <FileJson className="h-4 w-4" /> CSV preview
          </span>
          <ChevronDown className="h-4 w-4" />
        </CollapsibleTrigger>
  ...
  return (
    <Card className={fullSpan ? "md:col-span-2 lg:col-span-4" : ""}>
      <CardContent className="p-4">
        ...
        <div
          className={`mt-1.5 text-sm font-medium ${mono ? "font-mono" : ""}`}
    ...
    <div className="flex gap-1">
      {levels.map((l, i) => (
        <div
          ...
          style={{ backgroundColor: i <= idx && idx >= 0 ? l.color : "var(--muted)" }}
        />
      ))}
    </div>
  );
